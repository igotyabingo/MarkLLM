...
class KGW(BaseWatermark):
    ...
    def __init__(self, algorithm_config: Union[str, KGWConfig], transformers_config: Optional[TransformersConfig] = None, *args, **kwargs) -> None:
        """
    ...
    def generate_watermarked_text(self, prompt: str, *args, **kwargs) -> str:
        ...
        encoded_prompt = self.config.generation_tokenizer(prompt, return_tensors="pt", add_special_tokens=False).to(self.config.device)

        # Generate watermarked text
        encoded_watermarked_text = generate_with_watermark(**encoded_prompt)
        watermarked_text = self.config.generation_tokenizer.batch_decode(encoded_watermarked_text.sequences, skip_special_tokens=True)[0]
        generated_tokens_only = encoded_watermarked_text.sequences[0][-200:] 
        edited_text = self.config.generation_tokenizer.decode(generated_tokens_only, skip_special_tokens=True)

        return watermarked_text, encoded_watermarked_text.scores, edited_text, generated_tokens_only
    
    def detect_watermark(self, text: str, ids: torch.Tensor, return_dict: bool = True, *args, **kwargs): 
        """Detect watermark in the text."""
        if ids is None: # for natural text, detecting by
            ...
            z_score, green_token_flags = self.utils.score_sequence(ids)
