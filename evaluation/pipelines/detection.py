...
class WatermarkDetectionResult:
    ...
    def __init__(self, generated_or_retrieved_text, edited_text, detect_result) -> None:
        ...
        # self.red_entropy_mean = red_entropy_mean
        # self.green_entropy_mean = green_entropy_mean

        ## green flag 및 logit tensor 변수 추가
        pass
...
class WatermarkDetectionPipeline:
    ...
    def _detect_watermark(self, text: str, ids: torch.Tensor, watermark: BaseWatermark):
        """Detect watermark in text."""
        detect_result = watermark.detect_watermark(text = text, ids = ids, return_dict=True)
    ...
    def evaluate(self, watermark: BaseWatermark):
        ...
        for index in bar: # sample 단위
            prompt = self.dataset.get_prompt(index)
            generated_or_retrieved_text, scores_if_generated, edited_text, ids = self._generate_or_retrieve_text(index, watermark)
            detect_result = self._detect_watermark(generated_or_retrieved_text, ids, watermark)
...
class WatermarkedTextDetectionPipeline(WatermarkDetectionPipeline):
    ...
    def __init__(self, dataset, text_editor_list=[],
                 show_progress=True, return_type=DetectionPipelineReturnType.FULL, *args, **kwargs) -> None:
...
class UnWatermarkedTextDetectionPipeline(WatermarkDetectionPipeline):
    ...
    # natural = human text (golden), generated = unwatermarked-LLM
    ...
    def _generate_or_retrieve_text(self, dataset_index, watermark):
        ...
        if self.text_source_mode == 'natural':
            # ids 부분은 넘겨야 함
            return self.dataset.get_natural_text(dataset_index), None, None, None
        else:
            prompt = self.dataset.get_prompt(dataset_index)
